...
Lily Moreno has assigned us the first question for me to answer. Our business task is to analyze how annual members and casual riders use Cyclistic bikes differently in order to gain insight on how to effectively market to casual riders in order to increase annual membership.
...
Since the variable names are all the same, I combined all the month datasets into one dataframe using the bind_rows() function. This dataframe will be called all_trips. This made it easier for me to analyze all the data together without having to call all 12 datasets every time. 
...
I also added the ride_length column in order to create a variable to see how long each ride is by using difftime(). This would provide another variable that we can analyze. 

# add a ride_length calculation since there is no trip duration column
all_trips$ride_length<-difftime(all_trips$end_time,all_trips$start_time)

Next, I used the str() function to check the formatting of the columns. The ride_length column should be numeric in order to run calculations on it. I used the typeof() function in order to check the formatting of the ride_length column. The as.numeric() function changes the format to numeric and I used is.numeric() to make sure that ride_length is numeric.
...
Finally, there are some trips that have data that doesn’t make sense. Cyclistic performed quality checks on a certain number of bikes. So we must remove the trips that started at “HQ QR.” There are also some trips with negative ride_lengths, so we must remove them as well. I also used the na.omit() function to get rid of NA values. In doing this, the data frame that we use is now all_trips_v3.
...
To start analyzing the data, I used the summary() function to find the mean, median, minimum and maximum value of the all_trips_v3 data frame. This gives us more insight on the data and we can start to see where the data is trending. 
